...
1. 联系你的 Buildkite 组织创建者（管理员），请他在 Buildkite 官网创建队列：
   ...
   - 队列名称必须使用基础设施团队提供的标准名称（如 `ascend-a3`）
...
2. **Pipeline 基本配置**：
   - **Name**：为你的 Pipeline 起一个合适的名字（如项目名 + 用途，例如 `vllm-npu-ci`）
   - **Repository**：选择或输入你的 GitHub 仓库地址（如 `https://github.com/<org>/<repo>`）
   - **Default Branch**：设置默认触发分支（如 `main`）
   - **Pipeline Definition Source**：选择从仓库读取 Pipeline 定义
     - 选择 **Read pipeline definition from repository**
     - **File path**：填写 Pipeline 脚本文件路径，默认为 `.buildkite/pipeline.yml`
...
- Pipeline 的 **Source** 显示为从仓库读取（`.buildkite/pipeline.yml`）
